...
import { UserIntro, LeftsideContent, RightsideContent, Footer } from './components'
...
const UserProfile = () => {
  ...
  const publicNotes = (archivedNotes || []).filter((note) => note.notePublic === 1)
  const privateNotes = (archivedNotes || []).filter((note) => note.notePublic === 0)
    ...
    <Box className='w-full bg-[#4A4B51] h-auto'>
      <Box className='w-full bg-[#4A4B51] h-auto'>
        {userInfomations ? (
          ...
          <div className='flex justify-center items-center text-2xl font-bold'>Not found</div>
        )}
      </Box>
    </Box>
  )
