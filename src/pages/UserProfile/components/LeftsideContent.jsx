...
const LeftsideContent = ({ userInfomations, setReload }) => {
 ...
 useEffect(() => {
  setPublicNotes(archivedNotes.filter((note) => note.notePublic === 1))
  setPrivateNotes(archivedNotes.filter((note) => note.notePublic === 0))
  setPinnedNotes(
   archivedNotes.filter((note) => {
    if (user.id === userInfomations.id) {
     return note.pinned
    } else {
     return note.notePublic && note.pinned
    }
   })
  )
  setChecklistNotes(
    ...
    <Box className='flex-[4] w-full'>
     <TabContext value={publicNotesTabValue}>
      <div className='header-tabContent rounded-xl overflow-hidden mb-1'>
       ...
       >
        <TabList
         onChange={handlePublicNotesTabChange}
      ...
      <TabPanel value='1' className='w-full p-0'>
       {publicNotes.length > 0 ? (
        <Swiper
         ...
         style={{
          height: `${
           publicNotes.length > 2 ? 850 : publicNotes.length * 340
          }px`,
          width: '100%',
         }}
         navigation={{
            ...
            className={`h-[10rem] overflow-hidden px-4 py-2 rounded-lg
                                    ${
                                     user.id === userInfomations.id
                                      ? 'cursor-pointer hover:bg-opacity-20 hover:bg-gray-500 transition-colors duration-200'
                                      : ''
                                    }`}
            onClick={() =>
             user.id === userInfomations.id &&
             navigate(`/editnote/${info.idNote}`)
            }
           >
