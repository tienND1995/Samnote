...
const UserGroup = () => {
 ...
 useEffect(() => {
  const fetchMessageHistory = async () => {
   try {
    const response = await api.get(
     `https://samnote.mangasocial.online/message/list_message_chat1vs1/${user.id}/${dataInfomations.id}?page=1`
    )
    setMessage(response.data.data)
    console.log('successfully message history', response.data)
   } catch (error) {
    console.log(error.message)
   }
  }
 ...
 const handleClickChat = async (group) => {
  if (group.is_seen === 0) {
        ...
        return (
         ...
         >
          ...
          <div style={{ width: '100%' }}>
           ...
           >
            {handleLastText(item.last_text, item.idSend, item.last_image)}
           </p>
          </div>
   ...
   >
    ...
    >
     <div style={{ display: 'flex' }}>
      <Avatar
       ...
       src={dataInfomations ? dataInfomations.Avarta : selectedGroup.Avarta}
       alt=''
      />
     </div>
     <div
    ...
    >
     ...
     >
      ...
      <div
       className={imageContent !== null ? 'w-[100%] h-auto relative' : 'hidden'}
      >
       <img
      ...
      <IconButton
       ...
       className={
        messageContent === '' && imageContent === null && inputEmoji === null
         ? 'text-black'
         : 'text-[#1976d2]'
       }
      >
