...
<<<<<<< HEAD
  ...
  const senmess = {
   content: sendMess,
   idReceive: userInfo != null ? userInfo.id : part1 != user.id ? part1 : part2,
   idRoom: nowRoom ? nowRoom : `${user.id}#${userInfo.id}`,
     ...
     {dataSearch && (
      <Box className='max-h-[85vh] min-w-[100%] overflow-auto absolute bg-white px-1  z-[10] rounded-xl left-[50%] transform -translate-x-1/2 py-[30px]'>
       ...
       {dataSearch.status == 200 ? (
        ...
        <Typography variant='body2' sx={{ margin: '20px 0', color: 'black' }}>
...
>>>>>>> page/editnote
         ...
         />
         <Typography variant='body1'>{userInfomations.name}</Typography>
        </>
       ) : nowChat.user !== 'Unknow' ? (
        <>
         <Avatar
